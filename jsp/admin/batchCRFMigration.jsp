...
<!-- start of admin/batchCRFMigration.jsp -->
...
<!-- then instructions-->
	...
	<tr id="sidebar_Instructions_closed" style="display: all">
		<td class="sidebar_tab">
			...
			<div class="sidebar_tab_content"><fmt:message key="choose_crf_migration_batch_instruction_key" bundle="${resword}" /></div>
		</td>
	</tr>
...
<h1><span class="title_manage"> <fmt:message key="batch_crf_version_migration_for" bundle="${resworkflow}"/>&nbsp;<c:out value="${crf.name}"/></span></h1>
...
<div class="div_with_border">
	<form action="${pageContext.request.contextPath}/pages/api/v1/forms/migrate/run" method="post">
		<input type="hidden" name="studyOid" id="studyOid" value="${study.oid}">
		<input type="hidden" name="crfId" id="crfId" value="${crf.id}">
		<table>
			<tr>
				<td>
					<fmt:message key="current_version_of" bundle="${resword}"/>&nbsp;<c:out value="${crf.name}"/>&#58;
				</td>
			...
			<tr>
				<td>
					<fmt:message key="list_of_event_definitions" bundle="${resword}"/>&#58;
				</td>
				<td>
					<select name="selectedEvents" id="selectedEvents" multiple="multiple">
						<option value="<c:out value="-1" />" selected="selected">&nbsp;
							<c:out value="-All-" />&nbsp;
						</option>
			...
			<tr>
				<td>
					<input type="button" onclick="madeAjaxCall();" value="<fmt:message key="preview" bundle="${resword}"/>" class="button_long">
				</td>
				<td>
					<input type="submit" name="submit" style="display: none" value="<fmt:message key="migrate" bundle="${resword}"/>" class="button_xlong">
				</td>
			</tr>
		</table>	
	</form>
...
<!-- end of admin/batchCRFMigration.jsp -->
