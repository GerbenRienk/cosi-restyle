...
<c:choose>
	<c:when test="${userBean != null && userBean.id>0}">
		<tr id="sidebar_Info_open"<c:if test="${closeInfoShowIcons}">style="display: none"</c:if>>
			<td class="sidebar_tab">include/sideInfo.jsp<br />
			<a href="javascript:leftnavExpand('sidebar_Info_open'); leftnavExpand('sidebar_Info_closed');">*
			<img src="${pageContext.request.contextPath}/images/sidebar_collapse.gif" border="0" align="right" hspace="10"></a>
			...
			<c:choose>
				<c:when test="${panel.orderedData}">
					<c:forEach var='line' items="${panel.userOrderedData}">
						<b><c:out value="${line.title}" escapeXml="false"/>:</b>&nbsp;<c:out value="${line.info}" escapeXml="false"/><br /><br />
					</c:forEach>
				</c:when>
				<c:otherwise>
					<c:forEach var='line' items="${panel.data}">
						<b><c:out value="${line.key}" escapeXml="false"/>:</b>&nbsp;<c:out value="${line.value}" escapeXml="false"/><br /><br />
					</c:forEach>
				</c:otherwise>
			</c:choose>
		</div>
				...
				<c:if test="${panel.orderedData}">
					...
					<c:forEach var='line' items="${panel.userOrderedData}">
						...
						<c:if test="${!line.colon}">
							...
							<tr>
							<c:choose>
								<c:when test="${line.lastCRF}"><td valign="top" class="vline_B"></c:when>
								<c:otherwise><td valign="top" class="vline"></c:otherwise> 
							</c:choose>
		...
		<c:when test="${panel.submitDataModule}"><c:import url="../include/submitDataSide.jsp"/></c:when> 
		</c:choose>	
	</c:when>
	<c:otherwise>
		<br><br>
...
<script>leftnavExpand('sidebar_Info_open'); leftnavExpand('sidebar_Info_closed');</script>
<br /><img src="images/spacer.gif" width="120" height="1">
</td>
<td class="aka_revised_content" valign="top">
<!-- end include/sideInfo.jsp -->
